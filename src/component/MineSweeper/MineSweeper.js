...
const MineSweeper = () => {
  ...
  const createBoard = (rows, cols, minesCount) => {
    let board = Array(rows).fill().map(() => // tạo mảng mới với rows phần tử, hàm fill dùng để chèn null hoặc undefine và tất cả phần tử vừa được tạo. Map thì giống với foreach
      Array(cols).fill().map(() => ({ // thì mỗi phần tử trên sẽ chứa 1 phần tử khác cũng là dạng mảng, có thể hiểu mà mảng trong mảng. Ví dụ rows = 3, cols = 2 thì đầu tiên board sẽ có 3 phần tử, sau đó mỗi phần tử trong 3 phần tử đó sẽ chứa 1 mảng với 2 phần tử.
        hasMine: false, 
        isOpen: false,
        isFlagged: false,
        neighboringMines: 0,
    ...
    let minesPlaced = 0;
    while (minesPlaced < minesCount) { // phát đủ số mìn mình muốn thì ngừng
      const row = Math.floor(Math.random() * rows); // hàm này bên trò ColorTest có giải thích
      const col = Math.floor(Math.random() * cols);
      if (!board[row][col].hasMine) { // nếu ô được chọn từ random mà chưa có mìn thì sẽ set lại thuộc tính là đã có mìn
    ...
    for (let r = 0; r < rows; r++) { // chạy từng hàng
      for (let c = 0; c < cols; c++) { // chạy từng cột
        if (!board[r][c].hasMine) { // nếu mà ô nào không phải là mìn thì mới tính số mìn xung quanh
          let neighboringMines = 0;
          for (let i = -1; i <= 1; i++) { // vì sao lại chỉ có -1 đến 1, vì việc kiểm tra số mìn xung quanh chỉ cần đếm số mìn quanh mình, có nghĩa là cách mình 1 ô, for này là xét theo chiều dọc
            for (let j = -1; j <= 1; j++) { // như trên nhưng là chiều ngang
              if (r + i >= 0 && r + i < rows && c + j >= 0 && c + j < cols) { // kiểm tra ô đang được kiểm tra có nằm trong bảng không
                if (board[r + i][c + j].hasMine) { // if này sẽ đếm số mình xung quanh và cộng vào
                  neighboringMines++;
                }
              }
            }
  ...
  const handleClick = (row, col) => {
    ...
    } else if (board[row][col].neighboringMines === 0) { // nếu ô được chọn không có mìn thì mở các ô trống
  ...
  const openEmptyCells = (board, row, col) => {
    ...
    for (let i = 0; i < directions.length; i++) { // chạy từng vị trí xung quanh
      ...
      if (newRow >= 0 && newRow < rows && newCol >= 0 && newCol < cols) { // đảm bảo ô đang xét không vượt qua phạm vi của bảng
        if (!board[newRow][newCol].isOpen && !board[newRow][newCol].hasMine) { // nếu ô đó chưa mở và không có mìn thì mở
          board[newRow][newCol].isOpen = true;
          if (board[newRow][newCol].neighboringMines === 0) { // nếu ô vừa mở vẫn là 1 ô trống thì gọi đệ qui lại hàm để tiếp tục mở các ô trống liên tiếp
      ...
      {board.map((row, r) => (
        <div key={r} style={{ display: 'flex' }}>
          {row.map((cell, c) => 
            ...
            >
              {cell.isOpen ? (cell.hasMine ? '💣' : cell.neighboringMines || '') : (cell.isFlagged ? '🚩' : '')}
            </div>
          )}
        </div>
      ))}
